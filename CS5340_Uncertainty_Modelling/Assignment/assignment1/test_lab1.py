...
def test_observe_evidence():
    factors = [
        Factor(var=[0, 1], card=[2, 3], val=[0.4, 0.35, 0.6, 0.45, 0.0, 0.2]),
        Factor(var=[1, 2], card=[3, 2], val=[0.1, 0.2, 0.3, 0.9, 0.8, 0.7]),
    ]
    evidence = {0: 1}

    correct = [
        Factor(var=[0, 1], card=[2, 3], val=[0.0, 0.35, 0.0, 0.45, 0.0, 0.2]),
        Factor(var=[1, 2], card=[3, 2], val=[0.1, 0.2, 0.3, 0.9, 0.8, 0.7]),
    ]
...
def test_compute_marginals_naive():
    for graph in GRAPH_DATA:
        ...
        for test_case in test_cases:
            output = compute_marginals_naive(
                test_case["var"], factors, test_case["evidence"]
            )
...
def test_factor_sum():
    ...
    factor1 = Factor(var=[0, 1], card=[2, 2], val=[0.4, 0.55, 0.6, 0.45])
    correct = Factor(var=[0, 1], card=[2, 2], val=[1.2, 0.75, 1.4, 0.65])
...
@wrap_test
def test_factor_sum_1():
    # factorA contains phi(X_0)
    factor0 = Factor(var=[0], card=[2], val=[0.8, 0.2], val_argmax=[{2: 1}, {2: 0}])
    # factor1 contains phi(X_1|X_0)
    factor1 = Factor(var=[0, 1], card=[2, 2], val=[0.4, 0.55, 0.6, 0.45])
...
def test_map_eliminate():
    for graph in GRAPH_DATA:
        ...
        for test_case in test_cases:
            ...
            assert np.allclose(
                output[1], test_case["log_prob"]
            ), f"log_prob wrong, output {output[1]}, expected {test_case['log_prob']}"
            assert (
                output[0] == test_case["max_decoding"]
            ), f"Max decoding wrong, , output {output[0]}, expected {test_case['max_decoding']}"
